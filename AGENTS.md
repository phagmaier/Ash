...
`test/golden/demos.expected` stores what each prints. The collapse report is
`dune exec ash -- --collapse FILE [--depth N]`; `test/golden/collapse.expected`
stores it for a fixed sample set.
