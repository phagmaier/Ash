...
Regardless of choice: **timing, stack-depth, resource-exhaustion, gensym-counter, and span-provenance observation are excluded from the observational equivalence.** State this explicitly or the theorem is unfalsifiable. Provenance joined the list at task 7.3: a residual node records the phase that emitted it, so a failure the residual raises reports the same cause at the same *source* location under an extra generated layer. Requiring those layers to match would make every program that can fail count as changed by collapsing it — a claim about where a node came from wearing the clothes of a claim about what a program does. The source location and the cause are both still compared in full (ADR 0037).
...
**Done (task 7.3) — the split store is measured.** The effect-order corpus asks what mutation and output make askable: not whether a residual computes the same answer, but whether it does the same things, in the same order, ending in the same store. Fifteen programs state their observation as an answer plus the expressions that read the store back, so one run yields value, store and trace together; three more fail in a way the specializer had to leave in the residual, which is the only way the output and the writes *before* a failure become comparable at all. A closed program contains something dynamic by writing `deref(cell_new(v))`: allocation always residualizes, so the specializer cannot see the value and every run reads it as `v`. At each depth 0–5 the tower run and the *normalized* residual run agree on all four observations, and specialization writes nothing to the program's stream. Two failures agreeing had to be pinned down first: a residualized failure carries the specializer's provenance over the very span the source reported, so agreement compares the source location and the cause, never the generated layers — otherwise every program that can fail would be reported as changed by collapsing it. One boundary is recorded rather than hidden: a failure the specializer can decide, inside a branch it cannot, aborts the whole specialization instead of becoming a residual failure in that branch, so `if dynamic then 1/0 else 7` runs and does not collapse. Residualizing a decided failure — emitting a term that raises it where it stood — is error-and-control work no step of this ordering owns; the sample is asserted to be refused, so whichever phase takes it on announces itself by failing that assertion. ADR 0037.
...
- **Done:** effect policy is enforced (7.1), the store is split (7.2), and the corpus measures both (7.3). Nothing prints during compilation, and that is structural rather than incidental: the specializer refuses an always-residualizing class before it consults any rule that could fold, so a fold path added later inherits the refusal instead of having to remember it. `static_log` exists as this section prescribes — its own class, run when the specializer meets it, leaving no residual call, writing to a stream that is not the program's output — so wanting compile-time visibility is no longer a reason to reach for `print` (ADR 0035). `Set` then reached a residual for the first time: an abstract store keyed by cell identity decides per binding whether the specializer or the residual program owns it, forks at a conditional it cannot decide, and gives up any binding a branch writes before the fork so that both branches write to one place; holding is proved, not assumed, and a binding that cannot be proved is residualized rather than refused (ADR 0036). The corpus then made the claim rather than merely enabling it: 19 programs, 324 checks, depths 0–5, comparing value, observable store, output and failure between the depth-n tower and the normalized residual, with the specialization phase's own output required to be empty every time. The store is compared as what the program reads back out of it, because cells carry identity and no two runs allocate the same one (§D1); dynamic conditionals in closed programs are written `deref(cell_new(v))`, because allocation still residualizes; and two failures agree on cause and source location, provenance excluded, because a residualized failure necessarily carries the phase that emitted it (ADR 0037). Allocation primitives still residualize, so an `open fn` group is still interpretation the report counts, and one boundary is recorded rather than hidden: a decided failure inside an undecided branch stops the specialization instead of becoming a residual failure, which is §7.4 step 4's job.
...
- [x] Uniform stage-polymorphism → `print` at compile time. (D7) *(Addressed: the observable class is refused before any rule that could fold, `static_log` is the compile-time channel instead of an overloaded `print` (ADR 0035), and the effect-order corpus requires the specialization phase's own output to be empty at every depth (ADR 0037).)*
